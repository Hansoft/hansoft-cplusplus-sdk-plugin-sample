...
class CHansoftSDKSample_ClientPluginPlugin : public HPMSdkCallbacks
{
public:
	...
	CHansoftSDKSample_ClientPluginPlugin(const void *_pClientData)
	{
		m_pSession = NULL;
	...
	~CHansoftSDKSample_ClientPluginPlugin()
	{ 
		...
		try
		{
			if (_Data.m_SelectedTasks.size() > 0)
			{
				...
				m_pSession->GlobalAddRightClickMenuItem
					...
					, m_pSession->LocalizationCreateUntranslatedStringFromString(hpm_str("SDK sample plugin menu"))
					, NULL
					)
					;
#ifdef _MSC_VER
				...
				m_pSession->GlobalAddRightClickMenuItem
					...
					, m_pSession->LocalizationCreateUntranslatedStringFromString(hpm_str("Custom dialog..."))
					, NULL
					)
					;
			}
...
CHansoftSDKSample_ClientPluginPlugin *g_pClientPlugin;

extern "C" mod_export void DHPMSdkCallingConvention HPMClientSDKCreate(const void *_pClientData)
{
	g_pClientPlugin = new CHansoftSDKSample_ClientPluginPlugin(_pClientData);
}
